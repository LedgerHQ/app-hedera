...
bool hedera_derive_keypair(uint32_t index,
    ...
    if (CX_OK != cx_ecfp_init_private_key_no_throw(CX_CURVE_Ed25519, seed, sizeof(seed), &pk)) {
        MEMCLEAR(seed);
        return false;
    }

    if (public) {
        if (CX_OK != cx_ecfp_init_public_key_no_throw(CX_CURVE_Ed25519, NULL, 0, public)) {
            MEMCLEAR(seed);
            MEMCLEAR(pk);
            return false;
        }

        if (CX_OK != cx_ecfp_generate_pair_no_throw(CX_CURVE_Ed25519, public, &pk, 1)) {
            MEMCLEAR(seed);
            MEMCLEAR(pk);
            return false;
        }
...
bool hedera_sign(uint32_t index,
                 const uint8_t* tx,
                 uint8_t tx_len,
                 /* out */ uint8_t* result) {
    static cx_ecfp_private_key_t pk;
    ...
    if (CX_OK != cx_eddsa_sign_no_throw(&pk,        // private key
                                        CX_SHA512,  // hashID
