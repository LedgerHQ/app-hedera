...
static void rejectUseCaseChoice(void) {
    nbgl_useCaseConfirm("Reject message?", NULL, "Yes, reject",
                        "Go back to message", rejectChoice);
}
...
static void start_review(void) {
    ...
    nbgl_useCaseStaticReview(&layout, &review_final_long_press,
                             "Reject message", review_final_callback);
}
...
void ui_sign_transaction(void) {
    ...
    nbgl_useCaseReviewStart(
        &C_icon_hedera_64x64, "Review transaction to\nsign on Hedera\nnetwork",
        "", "Reject transaction", start_review, rejectUseCaseChoice);
