...
static const bagl_element_t ui_get_public_key_compare[] = {UI_BACKGROUND(),
...
static void shift_partial_key() {
    memmove(gpk_ctx.partial_key, gpk_ctx.full_key + gpk_ctx.display_index, DISPLAY_SIZE);
}

static unsigned int ui_get_public_key_compare_button(unsigned int button_mask, unsigned int button_mask_counter) {
...
static const bagl_element_t* ui_prepro_get_public_key_compare(const bagl_element_t* element) {
    if (element->component.userid == LEFT_ICON_ID && gpk_ctx.display_index == 0)
        return NULL;  // Hide Left Arrow at Left Edge
    if (element->component.userid == RIGHT_ICON_ID && gpk_ctx.display_index == KEY_SIZE - DISPLAY_SIZE)
...
UX_STEP_NOCB(ux_approve_pk_flow_1_step, bn, {"Export Public", gpk_ctx.ui_approve_l2});

UX_STEP_VALID(ux_approve_pk_flow_2_step, pb, io_seproxyhal_touch_pk_ok(NULL), {&C_icon_validate_14, "Approve"});

UX_STEP_VALID(ux_approve_pk_flow_3_step, pb, io_seproxyhal_touch_pk_cancel(NULL), {&C_icon_crossmark, "Reject"});

UX_DEF(ux_approve_pk_flow, &ux_approve_pk_flow_1_step, &ux_approve_pk_flow_2_step, &ux_approve_pk_flow_3_step);
